...
(unless something has been updated) and the only the services need to
be restarted, using

#+BEGIN_QUOTE
sudo /vagrant/scripts/setup.sh -S
#+END_QUOTE

Once the services have been restarted, the following should be
accessible:
...
pipelines, etc. need to be moved there.

/vagrant/virtuoso will contain the database (will be created once).
